...
def print_force_block(current_time, body_quat, quat_des, com_pos, foot_forces, Ag_mine, Ag_pin, mode="Centroidal"): # Function to print live force block
...
pin_model, _, _ = pin.buildModelsFromMJCF("/home/wrj/projects/centroidal_dynamics/go2_edit.xml")
...
physics.step() # Update dependent quantities

### >>> DEBUG AREA

# Site ID for CoM marker
com_site_id = model.name2id("com_marker", "site")
...
with mujoco.viewer.launch_passive(model.ptr, data.ptr) as viewer: # Simulation loop with viewer
    ...
    while viewer.is_running():
        ...
        if current_time < settling_duration: # Initial PD control for Each Leg
            ...
            # Ag_mine Calculation and Pinocchio Sync
            Ag_mine = compute_centroidal_momentum_matrix(model.ptr, data.ptr)
            ...
            print_force_block(current_time, body_quat, desired_quat, com_pos, foot_forces, Ag_mine, Ag_pin, mode="Settling (PD)")

       
        else:  # Centroidal Control
            
            com_pos = data.subtree_com[1] # CoM Position 
            com_vel = data.cvel[1, 3:6] # CoM Velocity
            body_quat = data.xquat[1]  # Base_link quaternion
            body_angvel = data.cvel[1, :3]  # Angular velocity
            ...
            for i, leg in enumerate(feet):
                ...
                # Torque contribution: r × F_i
                A[3:6, 3*i:3*i+3] = skew(r)

            # Desired wrench on body
            W_body = np.concatenate([total_force, total_torque])
            # Solve for foot forces: A @ F = -W_body
            F = np.linalg.pinv(A) @ (-W_body)
            # Assign to each leg
            for i, leg in enumerate(feet):
                foot_forces[leg] = F[3*i:3*i+3]
            ...
            print_force_block(current_time, body_quat, desired_quat, com_pos, foot_forces, Ag_mine, Ag_pin)

            #Update CoM site position AFTER stepping
            com_site_pos = data.subtree_com[0]
            data.site_xpos[com_site_id] = com_site_pos
